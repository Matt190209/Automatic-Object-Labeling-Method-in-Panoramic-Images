...
def train_model(model, data_loader, num_layers_to_freeze=2, num_epochs=100, weight_decay=0.0005, momentum=0.9):
    ct = 0
...
def predict(image, model, nms_threshold=0.3, resize_shape=(800, 800)):
    ...
    boxes, scores = apply_nms(np.array(boxes), np.array(scores), threshold=nms_threshold)
...
def get_true_labels(dataset, model):
    ...
    for image_path, image, target in dataset:
        ...
        # Si 'labels' no es una lista, conviértelo en una lista
        if not isinstance(labels, list):
            labels = [labels]
        # Si 'labels' es una lista de enteros, conviértelo en una lista de listas
...
def calculate_metrics(true_labels, pred_labels):
    ...
    true_labels_set = set(true_labels_flat)
    pred_labels_set = set(pred_labels)

    tp = len(true_labels_set.intersection(pred_labels_set))
    fp = len(pred_labels_set.difference(true_labels_set))
    fn = len(true_labels_set.difference(pred_labels_set))
...
def divide_into_cubemaps(image, cubemap_size=800):
    height, width, _ = image.shape
    cubemaps = []
    
    for i in range(0, height, cubemap_size):
        for j in range(0, width, cubemap_size):
            cubemap = image[i:i+cubemap_size, j:j+cubemap_size]
            if cubemap.shape[0] == cubemap_size and cubemap.shape[1] == cubemap_size:
...
def visualize_predictions(image_path, boxes, labels, scores, output_dir, threshold=0.2):
    image = cv2.imread(image_path)
    image = cv2.cvtColor(image, cv2.COLOR_BGR2RGB)
    fig, ax = plt.subplots(1, figsize=(12, 12))  # Ajustar el tamaño del gráfico
    ax.imshow(image)
    ...
    for box, label, score in zip(boxes, labels, scores):
        if score >= threshold:
            ...
            rect = patches.Rectangle((x_min, y_min), x_max - x_min, y_max - y_min,
                                     linewidth=2, edgecolor='r', facecolor='none')  # Incrementar el grosor de la línea
            ax.add_patch(rect)
    ...
    plt.savefig(output_path, bbox_inches='tight')  # Ajustar el gráfico para que se guarde correctamente
    plt.close(fig)
